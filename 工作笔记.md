...
`useRoute`函数用于获取当前路由对象，它返回一个包含当前路由信息的对象，包括路由名称、路由参数、查询参数等。你可以在组件中使用`useRoute`函数来访问当前路由对象，从而获取当前路由的信息并进行相应的处理。
...
[useMagicKeys](https://link.zhihu.com/?target=https%3A//vueuse.org/core/usemagickeys/) 监听按键状态,并提供了组合热键的功能,非常的神奇和有趣.使用它,我们可以很容易的监听我们使用CV大法的次数 :)
...
[useScroll](https://link.zhihu.com/?target=https%3A//vueuse.org/core/usescroll/) 对一些有性能要求的开发者提供了节流选项.但是我们希望的是用户有需求的时候才关注到有这个配置,因为当配置参数一多的时候,理解参数含义和配置其实是一种心智负担.另外,通用默认配置其实也是开箱能力的一种体现 !
...
从上面这个例子中我们可以看到,如果我们使用`reactive`的话,用户需要考虑解构会丢失响应性的问题,这也从一定程度上限制了用户使用解构的自由度和降低了这个函数的易用性.
...
VueUse 使用了 [VitePress](https://link.zhihu.com/?target=https%3A//github.com/vuejs/vitepress) 作为文档构建工具,下面我们来看下比较有意思的部分:
...
![在这里插入图片描述](https://img.jbzj.com/file_images/article/202111/2021111711524347.png)
...
### 过渡（transition）：哪个元素需要过渡就写在哪个元素上
...
> 若要从目前得状态回滚到 a0acb13c 得状态

![](C:\Users\shizeyu\Desktop\notes\Ajax-vue\Snipaste_2023-07-27_15-50-52.jpg)

运行：

```apl
git reset 复制得id
git reset 2b31aa8f1e5012911d897e1088e3261aec89324f
```

然后就能看到暂存区得暂存内容，对暂存进行处理（删除或者是那么）

然后使用命令 git push origin --force，就会覆盖远端
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20200310154209215.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MjQ0ODYyMw==,size_16,color_FFFFFF,t_70)
...
##### axios的处理链流程 [拦截器](https://so.csdn.net/so/search?q=拦截器&spm=1001.2101.3001.7020)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/2020031015565516.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20200310160412449.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MjQ0ODYyMw==,size_16,color_FFFFFF,t_70)

更多详情https://blog.csdn.net/weixin_42448623/article/details/104773280
...
## cascader、$confirm等弹出层样式修改失效的问题

[vue解决antd的cascader等弹出层样式修改失效的问题](https://blog.csdn.net/weixin_44090753/article/details/131232677?utm_medium=distribute.pc_relevant.none-task-blog-2~default~baidujs_baidulandingword~default-0-131232677-blog-91492181.235^v38^pc_relevant_sort&spm=1001.2101.3001.4242.1&utm_relevant_index=3)

官方api里面的`popupStyle`可以修改背景，颜色等，但是比如选项内容的hover，active样式就无法修改，即使使用`/deep/`穿透也不好使，原因是vue style中的`scoped`属性，无法修改动态加载的元素样式，但是如果去掉这个属性很可能会造成样式污染。

### 解决办法：

先设置`popupClassName`，再写一个不加[scoped](https://so.csdn.net/so/search?q=scoped&spm=1001.2101.3001.7020)的style:
...
// $comfirm 弹窗
<style lang="scss">
.test .confrim-sp1 .ant-modal-confirm-btns button:nth-child(1) {
  color: #0473ff;
...
this.$confirm({
        title: '提示',
        content: () => <div>确认取消年度合同？</div>,
        onOk: () => {
          annUpdateAgreeStatus({
            annualContractVo: {
              id: item.id,
              contractStatus: '05',
            },
          }).then(() => {
            _this.$message.success('合同取消成功');
            this.getTableData();
          });
        },
        onCancel() {
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/5c34938c103b4ff8b2de4f754c8095e6.png)
...
// model
<style scoped>
    .carbon-modal >>> .ant-modal-footer button:nth-child(1) {
      color: #0473ff;
      background-color: #f2f3f5;
      border: none;
    }
...
<font color='gree'>引用了第三方组件之后，如果需要在组件中局部修改第三方组件的样式，而又不想去除scoped属性造成组件之间的污染，引用>>>方式，就可以穿透scoped了。</font>

![](C:\Users\shizeyu\Desktop\notes\Ajax-vue\Snipaste_2023-10-05_13-58-42.jpg)

![](C:\Users\shizeyu\Desktop\notes\Ajax-vue\Snipaste_2023-10-05_13-59-17.jpg)
...
```js
{
          ...
          ellipsis: true,
          customRender: text => {
            return this.biaoqian.map(li => {
              return (
                <a-tooltip>
                  <template slot="title">
                    <span style="color: #939393; display: inline-block; background: #f3f3f3; padding: 3px;margin-right: 3px; border-radius: 2px">
                      {li}
                    </span>
                  </template>
                  <span style="display: inline-block; background: #f3f3f3; padding: 3px;margin-right: 3px; border-radius: 2px">
                    {li}
                  </span>
                </a-tooltip>
              );
...
----------------------------
// 仔细观察两钟不同情况，<> 需要加（），js语法要加{}。有时候也需要上面反复return的情况
customRender: text => {
            return (
              <a-tooltip>
                <template slot="title">
                  {this.biaoqian.map(li => (
                    <div style="color: #939393; display: inline-block; background: #f3f3f3; padding: 3px; margin: 1px 3px;; border-radius: 2px">
                      {li}
                    </div>
                  ))}
                </template>
...
> 
>
> features/08-stationDropDownSecond
>
...
> http://localhost:8080/#/myOrder/index
>
> http://localhost:8080/#/third_party/purchase
>
> 1
>
...
>
> ```js
>// 卖家推单：买家中心-收到的报价单（我的报价单）-立即下单（或点击进入详情页-立即下单）
> // 这个是tripartite-tree-selector换stationSelector
>{
> meta: {
>title: '立即下单',
> },
> path: '/third_party/purchase',
> name: 'purchase',
> component: () => import('@/views/thirdParty/purchase'),
> },
> 
> 
> 
> // 秒杀
> //实际还是修改/components/delivery-schedule/index的
> // TreeSelector换NewTreeSelector
> {
> meta: {
> title: '填写采购订单',
> },
> path: '/deal-add',
> name: 'deal_add_index',
> component: () => import(/* webpackChunkName: 'deal' */ '@/views/deal/add'),
> },
> 
> 
> // 现货挂牌(1.0)
...
>    title: '填写交货信息', // 现货
>      },
>  path: '/deal-add-spotgoods-delivery',
>  name: 'deal_add_delivery_information',
>  component: () => import(/* webpackChunkName: 'deal' */ '@/views/deal/add-delivery-spotgoods'),
>    },
>    
>    
>    // 
...
> },
> path: '/deal-sales-add',
> name: 'deal_sales_add_index',
> component: () => import(/* webpackChunkName: 'deal' */ '@/views/deal/sales-add'),
> },
> 
> {
> meta: {
...
> path: '/zhoushan/info',
> name: 'info',
> component: () => import('@/views/zhoushan/info'),
> },
> ```
> 
> ```js
> // 组件@/components/tree-selector/tripartiteTreeSelector改为
> // @/components/tree-selector/stationSelector
> ```
> 
...
> ```js
>// pc -base 2.0
> // 交易管家推单 卖家中心-订单管理-销售订单-创建
> /create/protocol?factoryType=01
> {
...
>       permisId: '100096',
>       pPermisId: '100004',
>       parents: ['100026', '100096'],
>     },
>     component: () =>
>       import(/* webpackChunkName: 'manufactor' */ '@/components/manufactor/createProtocol'),
>    },
>    // 文件地址      /components/manufactor/createProtocol
>   ```
> 
> ![image-20230717102851112](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230717102851112.png)
>
> ```js
>// pc 现货挂牌 团购 -greatgas-vue-lng-flagship
> // 舟山大厅- 挂牌-下单
> zhoushan账户：
> https://test.greatgas.cn/gastrade/#/tank-info?goodsInstId=7D68D66CB1E15199362DB549B8C93E35&prisenume=22&companyName=%E5%98%89%E5%85%B4%E7%87%83%E6%B0%94%E6%97%97%E8%88%B0%E5%BA%97&salesStatus=07&cid=0E1035D8F74B86115606849702ADB810&areaPriceId=&freightShow=0&stationRequired=1&stationSaleScope=0&t=1689555119328
> https://dev.greatgas.cn/gastrade/#/tank-info?goodsInstId=F881F0D28BC5E688B0887C76AF1400DC&prisenume=22&companyName=%E8%88%9F%E5%B1%B1%E4%BA%A4%E6%98%93%E5%A4%A7%E5%8E%85&salesStatus=07&cid=&areaPriceId=&freightShow=1&stationRequired=1&stationSaleScope=1&t=1689561241012
> http://localhost:8082/#/tank-info?goodsInstId=B08CC291F17E9DD8AD6BD15F433CFB16&prisenume=22&companyName=%E8%88%9F%E5%B1%B1%E4%BA%A4%E6%98%93%E5%A4%A7%E5%8E%85&salesStatus=14&cid=&areaPriceId=&t=1689575585296
> {
>  path: '/tank-info',
>  name: 'tank_info',
>     meta: {
...
>     },
>     component: () => import(/* webpackChunkName: 'tank' */ '@/views/tank/info'),
>    },
>    // 文件地址      /views/tank/info
>   // 更改组件 src/components/public/AddBuyNum      -非现货 2.0
> // 更改组件 src/components/public/AddBuyNumZS    -现货
> ```
> 
> ![image-20230717103536387](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230717103536387.png)
...
>       menuShow: false,
>     },
>     component: () => import(/* webpackChunkName: 'buyer' */ '@/views/myquote/purchaseOrder'),
>   },
> ```
>
> ![image-20230717102653208](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230717102653208.png)
>
> ![image-20230717102825222](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230717102825222.png)

```js
 // 存疑
pc
/sellerCenter/createSaleOrder
{
    path: '/sellerCenter/createSaleOrder',
    ...
    component: () =>
      import(
        /* webpackChunkName: 'manufactor' */ '@/views/sellerCenter/mySaleOrder/createSaleOrder'
      ),
  },
```

![image-20230717102955854](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230717102955854.png)
...
```vue
<template>
  <div class="commodity-listing">
    <div class="commodity-listing-head">
      <div>创建挂牌销售订单</div>
    </div>
    <div class="commodity-listing-body">
      <a-form-model
        ref="ruleForm"
        :model="form"
        :rules="rules"
        ...
        <div style="margin: 0 20px; border-bottom: 1px solid #e2e2e2">
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="是否分卸" prop="ifUnloading">
                <a-radio-group v-model="ifUnloading" @change="separateUnloadingFun">
                  <a-radio value="02">否</a-radio>
                  <a-radio value="01" v-if="factoryType != '02'">同一买家分卸</a-radio>
                </a-radio-group>
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="客户名称" prop="buyerFirmName">
                <a-select
                  show-search
                  placeholder="请输入关键字搜索"
                  v-model="form.buyerFirmName"
                  ...
                  <a-select-option v-for="item in selectList" :key="item">
            ...
            <a-col :span="12">
              <a-form-model-item label="结算单位" prop="settlementUnit">
                <span>{{ form.settlementUnit }}</span>
              </a-form-model-item>
            </a-col>
          </a-row>
        </div>
        <div class="theme"><span></span>采购信息</div>
        <a-row>
          <a-col :span="12">
            <a-form-model-item
              :label-col="{ span: 5 }"
              ...
              label="商品"
              prop="goodsName">
              <a-select v-model="form.goodsName" placeholder="请选择商品" @change="changeGoodname">
                <a-select-option v-for="item in goodsList" :key="item.goodsInstId"
                  >{{ item.proTitle }}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          ...
          <a-col :span="12">
            <a-form-model-item
              ...
              label="交货方式"
              prop="deliveryMethod">
              <a-radio-group v-model="form.deliveryMethod" @change="handleDeliveryMethod">
                <a-radio value="01" v-if="goodsDeliveryCode.includes('01')">自提</a-radio>
                <a-radio value="02" v-if="goodsDeliveryCode.includes('02')">配送</a-radio>
              </a-radio-group>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item
              ...
              <a-radio-group v-model="form.payMethod">
                <a-radio value="01" :disabled="disburseType">预付</a-radio>
                <a-radio value="02" :disabled="!disburseType">后付</a-radio>
              </a-radio-group>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item
              ...
              :wrapper-col="{ span: 17 }"
              label="支付方式"
              prop="supportPayMethods">
              <a-radio-group v-model="form.supportPayMethods">
                <a-radio value="02">线下支付</a-radio>
                <a-radio value="01" :disabled="!isOpenPayment">线上支付</a-radio>
              </a-radio-group>
            </a-form-model-item>
          </a-col>
        </a-row>
        <!-- 自提站点 -->
        <div v-if="!separateUnloadingShow" class="price-information" style="padding: 15px 0">
          <a-row :gutter="16" class="from-information-head" style="margin-bottom: 12px">
            <a-col :span="6">购买数量</a-col>
            <a-col :span="9">用气场站</a-col>
            <a-col :span="9">期望到货时间</a-col>
          </a-row>
          <div>
            <a-row :gutter="16">
              <a-col :span="6">
              ...
              <a-col :span="9">
                <a-form-model-item
                  ...
                  <a-cascader
                    changeOnSelect
                    style="width: 100%"
                    v-model="form.stationIdarr"
                    :options="memberStationList"
                    :show-search="{}"
                    @popupVisibleChange="dropdownVisibleChangeOpen"
                    @change="cascaderChangeBefore"
                ...
                <div
                  v-if="
                    form.transportMileage &&
                    form.stationIdarr[0] &&
                    form.deliveryMethod.includes('02')
                  "
                  class="transport_mileage unloading">
                  <span
                    ><img
                      width="11px"
                      height="11px"
                      src="@/assets/imager/transport_mileage.png"
                      alt="mileage"
                  /></span>
                  <span> 配送里程：{{ form.transportMileage.toFixed(3) }}公里</span>
                </div>
              </a-col>
              <a-col :span="9">
                <a-form-model-item
                  ...
                  <a-date-picker
                    ...
                    :show-today="false"
                    @change="pickerChange()"
                    @openChange="() => (panelMode = 'date')"
                    @panelChange="(value, mode) => (panelMode = mode)"
                    :placeholder="`请选择日期`" />

                  <a-time-picker
                    v-model="form.suppDateT"
                    style="width: 40%"
                    format="HH:mm"
                    :disabled="planTimedis"
            ...
            <a-row :gutter="16">
              <a-col :span="6">
                <a-form-model-item
                  ...
                  <a-input
                    :maxLength="20"
                    v-model="form.DeliveryTerm[0].contact"
                    placeholder="请输入联系人" />
                </a-form-model-item>
              </a-col>
              <a-col :span="9">
                <a-form-model-item
                  ...
                  <a-input
                    :maxLength="20"
                    v-model="form.DeliveryTerm[0].contactTel"
                    @blur="numberVerification(form.DeliveryTerm[0].contactTel)"
                    placeholder="请输入联系电话" />
                </a-form-model-item>
              </a-col>
              <a-col :span="9">
                <a-form-model-item
                  ...
                  <a-input
                    :maxLength="50"
                    v-model="form.DeliveryTerm[0].remark"
                    placeholder="请输入备注" />
                </a-form-model-item>
              </a-col>
            </a-row>
        ...
        <!-- 分卸站点 -->
        <div v-if="separateUnloadingShow" class="price-information" style="padding: 15px 0">
          <a-row :gutter="16" class="from-information-head">
            <a-col :span="4">购买数量</a-col>
            <a-col :span="9">分卸场站</a-col>
            <a-col :span="9">期望到站时间</a-col>
            <a-col :span="2" style="text-align: center">操作</a-col>
          </a-row>
          <div v-for="(item, index) in form.DeliveryTerm" :key="item.id">
            <a-row :gutter="16" style="margin-bottom: 15px">
              <a-col :span="4">
                <a-input
                  ...
                  onkeyup="value=value.replace(/^(0+)|[^\d]+/g,'')"
                  :maxLength="8"
                  @blur="goosquantinput(index)" />
              </a-col>
              <a-col :span="9">
                <a-cascader
                  changeOnSelect
                  v-model="item.stationIdarr"
                  :options="memberStationList"
                  :show-search="{}"
                  @popupVisibleChange="dropdownVisibleChangeOpen($event, item)"
                  @change="termChangeBefore(item, index, $event)"
                  :field-names="{ label: 'text', value: 'id', children: 'children' }"
                  placeholder="请选择用气场站"
                  style="width: 100%"
                  class="addyqzd" />
                <!-- v-if="item.transportMileage" -->
                <div v-if="item.transportMileage && item.stationIdarr[0]" class="transport_mileage">
                  <span
                    ><img
                      width="11px"
                      height="11px"
                      ...
                      alt="mileage"
                  /></span>
                  <span> 配送里程：{{ item.transportMileage.toFixed(3) }}公里</span>
                </div>
              </a-col>
              <a-col :span="9">
                <a-date-picker
                  ...
                  :show-today="false"
                  @change="termpickerChange(index)"
                  @openChange="() => (panelMode = 'date')"
                  @panelChange="(value, mode) => (panelMode = mode)"
                  :placeholder="`请选择日期`" />

                <a-time-picker
                  v-model="item.suppDateT"
                  style="width: 40%"
                  format="HH:mm"
                  :defaultOpenValue="$moment('00:00', 'HH:mm')"
                  :disabledMinutes="disabledMinutes"
                  hideDisabledOptions
                  :disabled="item.planTimedis"
                  @change="termpickerTimeChange(index)"
                  :placeholder="`选择时间`" />
              </a-col>
              <a-col :span="2" style="text-align: center">
                <!-- <a-button
                  type="link"
                  v-show="form.DeliveryTerm.length > 1 && index != 0"
            ...
            <a-row :gutter="16">
              <a-col :span="4">
                <a-form-model-item
                  ...
                  :label-col="{ span: 8 }"
                  :wrapper-col="{ span: 16 }">
                  <a-input :maxLength="20" v-model="item.contact" placeholder="请输入联系人" />
                </a-form-model-item>
              </a-col>
              <a-col :span="9">
                <a-form-model-item
                  ...
                  <a-input
                    ...
                    @blur="numberVerification(item.contactTel)"
                    placeholder="请输入联系电话" />
                </a-form-model-item>
              </a-col>
              <a-col :span="9">
                <a-form-model-item
                  ...
                  :label-col="{ span: 3 }"
                  :wrapper-col="{ span: 21 }">
                  <a-input :maxLength="50" v-model="item.remark" placeholder="请输入备注" />
                </a-form-model-item>
              </a-col>
              <a-col :span="2" style="text-align: center">
                <a-button
                  ...
                  v-show="form.DeliveryTerm.length > 1 && index != 0"
                  @click="handleDelete(item)"
                  style="line-height: 42px; color: #005195"
                  >删除
                </a-button>
              </a-col>
            </a-row>
          </div>
          <div class="price-information-foot" @click="handleAdd">
            <a-icon type="plus-circle" />
            新增一条交货需求
          </div>
        </div>
        <!-- 分卸里程 -->
        <!-- <a-row v-if="separateUnloadingShow">
          <a-col :span="12">
            <a-form-model-item label="分卸里程区间" prop="mileageval">
              <a-select
                v-model="form.mileageval"
                placeholder="请选择分卸里程区间"
                @change="selectChange">
                <a-select-option
                  v-for="item in fxFeeColumns"
                  :key="item.unloadCostId"
                  :value="`${item.unloadCostId}//${item.unloadCostCn}//${item.price}`">
                  {{ item.unloadKilometreCn }}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          </a-col>
          <a-col :span="12">
            <a-form-model-item label="分卸费用" prop="unloadCostCn">
              <a-input v-model="form.unloadCostCn" disabled />
            </a-form-model-item>
          </a-col>
        </a-row> -->
        <a-row :gutter="16">
          <a-col :span="24">
            <a-form-model-item
              :label-col="labelSmall"
              :wrapper-col="wrapperSamll"
              label="交易备注"
              prop="remark">
              <a-textarea v-model="form.remark" :maxLength="200" />
            </a-form-model-item>
          </a-col>
        </a-row>
        <div v-if="deliveryMethodShow">
          <div class="theme"><span></span>交易约定</div>
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="压车标准" prop="stdDelay">
                <a-select v-model="form.stdDelay" placeholder="请选择压车标准" :disabled="true">
                  <a-select-option value="01">6小时</a-select-option>
                  <a-select-option value="02">12小时</a-select-option>
                  <a-select-option value="03">1天</a-select-option>
                  <a-select-option value="04">2天</a-select-option>
                  <a-select-option value="05">3天</a-select-option>
                  <a-select-option value="06">4天</a-select-option>
                  <a-select-option value="07">5天</a-select-option>
                  <a-select-option value="08">6天</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :span="12">
              <a-form-model-item label="压车处理" prop="delayPrice">
                <a-input v-model="form.delayPrice" suffix="元/小时/车" :disabled="true" />
              </a-form-model-item>
            </a-col>
          </a-row>
          <a-row>
            <a-col :span="12">
              <a-form-model-item label="气差范围" prop="gasheadStdRange">
                <a-select
                  v-model="form.gasheadStdRange"
                  placeholder="请选择气差范围"
                  :disabled="true">
                  <a-select-option value="01">±200公斤</a-select-option>
                  <a-select-option value="02">±150公斤</a-select-option>
                  <a-select-option value="03">±100公斤</a-select-option>
                  <a-select-option value="04">±50公斤</a-select-option>
                  <a-select-option value="05">±0公斤</a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
            <a-col :span="12">
              <a-form-model-item label="气差处理" prop="gasheadStdGap">
                <a-select
                  v-model="form.gasheadStdGap"
                  placeholder="请选择气差处理"
                  :disabled="true">
                  <a-select-option value="01">双方协商</a-select-option>
                  <a-select-option value="02">按照实际卸气量结算</a-select-option>
                  <a-select-option value="03">按照实际加气量结算</a-select-option>
                  <a-select-option value="04"
                    >按照（实际复核结果+最大允许计量误差标准）结算
                  </a-select-option>
                </a-select>
              </a-form-model-item>
            </a-col>
        ...
        <div class="theme"><span></span>价格信息</div>
        <div class="price-information" v-if="!separateUnloadingShow">
          <a-row>
            <a-col :span="12">
              <!-- <p class="price-information-exclusive-price">
                ：￥{{ agreePromotion.promotionDelList[0].discountPrice }}/吨
                <a class="marginl" @click="handleDetails(false)">优惠详情</a>
              </p> -->
              <a-form-model-item
                :label-col="{ span: 5 }"
                :wrapper-col="{ span: 17 }"
                label="专享价"
                prop="marginl">
                <a-input v-model="agreePromotion.hasFareDiscountPrice" suffix="元/吨" disabled />
                <a
                  class="marginl"
                  style="position: absolute; top: -12px; right: -58px; display: block"
                  @click="handleDetails(false)"
                  >优惠详情</a
                >
              </a-form-model-item>
            </a-col>
            <a-col :span="12" v-if="goosnameList.freightShow === 1">
              <a-form-model-item
                :label-col="{ span: 7 }"
                :wrapper-col="{ span: 17 }"
                label="运价"
                prop="fare">
                <a-input v-model="form.fare" suffix="元/吨" disabled />
              </a-form-model-item>
            </a-col>
            <a-col :span="12">
              <a-form-model-item
                :label-col="{ span: goosnameList.freightShow === 1 ? 5 : 7 }"
                :wrapper-col="{ span: 17 }"
                label="其他优惠"
                prop="otherDiscount">
                <a-input
          ...
          <div class="price-information-price">
            <p>
              订单价：￥<span
                >{{ (parseFloat(price) && parseFloat(price).toFixed(2)) || '0.00' }} /吨</span
              >
            </p>
            <p>
              成交量：<span>{{ form.goodsQuantity }} 吨</span>
            </p>
            <p>
              结算金额：<b class="price">￥</b
              ><span class="price">{{ form.settlementAmount }}</span>
            </p>
          </div>
        </div>
        <div class="price-information" v-if="separateUnloadingShow">
          <a-row class="price-information-head">
            <a-col :span="3">场站</a-col>
            <a-col :span="6">专享价（元/吨）</a-col>
            <a-col :span="4" v-if="goosnameList.freightShow === 1">运价（元/吨）</a-col>
            <!-- <a-col :span="2">分卸费</a-col> -->
            <a-col :span="4">其他优惠（元/吨）</a-col>
            <a-col :span="3">备注</a-col>
            <a-col :span="2">小计（元）</a-col>
          </a-row>
          <a-row
            class="price-information-body"
            v-for="(item, index) in form.DeliveryTerm"
            :key="item.id">
            <a-col :span="3">
              <p>{{ item.stationname }}</p>
            </a-col>
            <a-col :span="6">
              <p class="footer-exclusive-price" style="width: 90%">
                专享价：￥{{ item.hasFareDiscountPrice }}/吨
                <a class="marginl" @click="handleDetails(true, index)">优惠详情</a>
              </p>
            </a-col>
            <a-col :span="4" v-if="goosnameList.freightShow === 1">
              <a-form-model-item label="">
                <a-input v-model="item.fare" disabled />
              </a-form-model-item>
            </a-col>
            <!-- <a-col :span="2">{{ form.fxfeeprise }}元/吨</a-col> -->
            <a-col :span="4">
              <a-form-model-item label="">
                <a-input
                  v-model="item.otherDiscount"
                  @input="otherdisinput(index)"
                  @blur="termoherblur(index)" />
              </a-form-model-item>
            </a-col>
            <a-col :span="3">
              <a-form-model-item label="" :wrapper-col="{ span: 23, offset: 0 }">
                <a-tooltip>
                  <template slot="title">
                    {{ item.remark }}
                  </template>
            ...
            <a-col :span="2">{{ item.cifcost }}</a-col>
          </a-row>
          <div class="price-information-foot turnover">
            <span>成交额</span> <b>￥{{ form.cifcostTotal }}</b>
          </div>
        </div>
        <a-form-model-item :wrapper-col="{ span: 24, offset: 0 }" class="footer">
          <a-button class="btn-add" @click="onSubmit" :disabled="butdisabled">提交</a-button>
          <a-button class="btn-baoc" @click="handleCancel">取消</a-button>
        </a-form-model-item>
      </a-form-model>
    </div>
    <div>
      <a-modal
        ...
        <div style="padding: 10px">
          <offerDetails
            :list="offerDetailsData"
            :price="price"
            :creatshow="true"
            :showFreight="goosnameList.freightShow === 1"></offerDetails>
          <div class="center">
            <a-button type="primary" @click="offerDetailsShow = false">关闭</a-button>
          </div>
        </div>
      </a-modal>
    </div>
    <!-- 候补订单弹窗 -->
    <SelectAlternate
      :visible.sync="alternateshow"
      :agreeDelStockList="agreeDelStockList"
      :type="type"></SelectAlternate>

    <!-- 加载中 -->
    <div>
      <div class="example" v-if="showSpin">
        <a-spin />
      </div>
    </div>
  </div>
...
<script>
import offerDetails from '../public/offerDetails';
import SelectAlternate from '../public/SelectAlternate';
import {
  findMemberListByCustomer,
  toAdd,
  findMemberStationByCustomerIdAndSource,
  findMemberStationInfo,
  calcAgreePromotion,
  getUnloadCost,
  createEntrustAgree,
  // getFareList,
  ...
  isOpenPayment,
  stock,
  getMutiSettlementByBuyer,
} from '@/http/manufactor';
import { disabledMinutes } from '@/utils/index';

export default {
  name: 'createProtocol',
  components: { offerDetails, SelectAlternate },
  data() {
    return {
      type: 1,
      disabledList: [],
      ...
      wrapperLong: { span: 24 },
      labelSmall: { span: 2 },
      wrapperSamll: { span: 22 },
      deliveryMethodShow: false,
      showSpin: false,
      agreeDelStockList: {},
      separateUnloadingShow: false,
      butdisabled: false,
      offerDetailsShow: false,
      factoryType: '01', //01:舟山厂家、02:罐箱厂家、03:旗舰店厂家 如果是02，分卸就隐藏。
      startdis: true,
      offerDetailsData: [
        {
          name: '挂牌价',
      ...
      selectList: [],
      disburseType: false,
      planTimedis: true, //计划到站时间时分秒默认不可选
      goodsList: [],
      goodsDeliveryCode: '#01#02', //交货方式
      memberStationList: [], //用气场站
      stationInfo: {}, //用气场站的详情
      ifUnloading: '02',
      fxFeeColumns: [], //分卸里程列表
      temporaryFxFee: '',
      ...
      form: {
        ...
        goodsQuantity: '',
        payMethod: '01',
        supportPayMethods: '02',
        mileageval: [], //分卸里程
        deliveryTime: '',
        unloadCostCn: '', //分卸费用
        unloadPathStationIdName: '',
        remark: '', //交易备注
        estimateDate: '',
        suppDate: '',
        suppDateT: '',
        ...
        gasheadStdGap: '',
        fare: '0', //运价
        otherDiscount: 0, //其他优惠
        agreementPrice: '0.00',
        Turnover: '20',
        settlementAmount: '0.00',
        cifcostTotal: '0.00', //成交额
        DeliveryTerm: [
          {
            id: '4325678098',
            stationIdarr: [],
      ...
      rules: {
        buyerFirmName: [{ required: true, message: '客户名称不能为空', trigger: 'blur' }],
        ifUnloading: [{ required: true, message: '是否分卸不能为空', trigger: 'change' }],
        gasSourcesName: [{ required: true, message: '气源地不能为空', trigger: 'change' }],
        deliveryMethod: [{ required: true, message: '交货方式不能为空', trigger: 'change' }],
        goodsName: [{ required: true, message: '商品名称不能为空', trigger: 'change' }],
        planTime: [{ required: true, message: '计划到站时间不能为空', trigger: 'change' }],
        stationIdarr: [{ required: true, message: '用气场站不能为空', trigger: 'change' }],
        stdDelay: [{ required: true, message: '压车标准不能为空', trigger: 'change' }],
        delayPrice: [{ required: true, message: '压车处理不能为空', trigger: 'blur' }],
        gasheadStdRange: [{ required: true, message: '气差范围不能为空', trigger: 'change' }],
        gasheadStdGap: [{ required: true, message: '气差处理不能为空', trigger: 'change' }],
        goodsQuantity: [{ required: true, message: '购买数量不能为空', trigger: 'blur' }],
        productPicture: [{ required: true, message: '商品图片不能为空', trigger: 'blur' }],
        MinimumPurchase: [{ required: true, message: '最低采购不能为空', trigger: 'blur' }],
        transportation: [
          { required: true, message: '运输标准不能为空', trigger: 'blur' },
          { min: 1, max: 8, message: '数量请输入1-8位', trigger: 'blur' },
        ],
        supportPayMethods: [{ required: true, message: '支付方式不能为空', trigger: 'change' }],
        payMethod: [{ required: true, message: '付款方式不能为空', trigger: 'change' }],
        fare: [{ required: true, message: '运价不能为空', trigger: 'blur' }],
        otherDiscount: [{ required: true, message: '其他优惠不能为空', trigger: 'blur' }],
        mileageval: [{ required: true, message: '请选择分卸里程区间', trigger: 'change' }],
      },
      memberId: '', //买家id
      goosnameList: {}, //选中的商品
      factoryId: '', //厂家id
      memberList: {},
      isOpenPayment: true, //是否开通线上支付
      agreePromotion: {
        promotionDelList: [
          {
            discountPrice: 0,
  ...
  created() {
    //判断浏览器参数
    if (this.$route.query.factoryType) {
  ...
  methods: {
    ...
    init() {
      //初始化
      toAdd().then(res => {
        this.goodsList = res.data.results.data.goodsList;
      });
    },
    numberVerification(valx) {
      const patrn = /^1[3456789]\d{9}$/; //手机电话
      const zjdh = /^((0\d{2,3})-)?(\d{7,8})$/; //座机电话
      if (valx && patrn.test(valx) == false && zjdh.test(valx) == false) {
        this.$message.warning('请输入正确的站点联系电话！');
        return false;
      } else {
        return true;
      }
    },
    // 根据挂牌商品类型和选择的气源地查询对应结算主体
    getMutipleSettlement() {
      if (this.form.buyerFirmName && this.form.goodsName) {
        getMutiSettlementByBuyer({
          buyerCustomerId: this.memberList.memberCustomerId,
          goodInstId: this.form.goodsInstId,
        }).then(res => {
          const { sellerInfo } = res.data.results;
          this.form.settlementUnit = sellerInfo.customerExt.companyName;
          this.form.sellerId = sellerInfo.customerId;
          this.form.sellerFirmName = sellerInfo.customerExt.companyName;
          this.form.sellerName = sellerInfo.customerName;
          this.form.sellerFirmId = sellerInfo.customerExt.orgId;
        });
      }
    },
    handleDeliveryMethod() {
      // 交货方式
      //【交货方式】更新交货方式，清空用气场站、清空专享价、清空运价、交易约定的显示和隐藏
      this.form.stationIdarr = [];
      this.form.stationId = '';
      this.cleraprice(); //清空价格
      this.clearMileageValue();
      if (this.form.deliveryMethod == '01') {
        this.deliveryMethodShow = false;
        console.log('===11==', this.goosnameList);

        //场站非自填 清空场站所有数据
        if (this.goosnameList.stationRequired == 0) {
          this.rules.stationIdarr = [
            { required: false, message: '用气场站不能为空', trigger: 'change' },
          ];
          this.$refs.ruleForm.clearValidate(['stationIdarr']);

          this.form.transportMileage = '';
          this.stationInfo = [];
          this.form.DeliveryTerm.map(res => {
            res.stationId = '';
            res.contact = '';
            res.contactTel = '';
            res.remark = '';
        ...
        this.deliveryMethodShow = true;
        this.rules.stationIdarr = [
          { required: true, message: '用气场站不能为空', trigger: 'change' },
        ];
      }
      this.form.stationIdarr = [];
      this.fndMemberStation();
      //判断查询价格用气场站，交货方式，商品，客户名称
      if (this.goosnameList && this.form.deliveryMethod && this.form.stationId && this.memberId) {
        this.priceListinit();
      }
      //判断是否查询运费 商品，用气场站，计划到站时间（交货时间）配送
      if (
        // this.goosnameList.ifcontainsFreight == 'false' &&
        this.form.stationId &&
        this.form.suppDate
        // this.form.suppDate &&
        // this.form.deliveryMethod == '02'
      ) {
        this.getFareList();
      }
    },
    separateUnloadingFun() {
      // 是否分卸
      //清空数据
      this.$refs.ruleForm.resetFields();
      this.startdis = true;
      this.planTimedis = true;
      this.form.suppDateT = '';
      this.form.suppDate = '';
      this.form.stationIdarr = [];
      this.form.stationId = '';
      this.form.stdDelay = '';
      this.form.delayPrice = '';
      this.form.gasheadStdRange = '';
      this.form.gasheadStdGap = '';
      this.form.payMethod = '01';
      this.form.fxfeeprise = 0;
      this.form.transportMileage = 0;
      this.goodsDeliveryCode = '#01#02'; //交货方式
      this.cleraprice(); //清空价格
      this.clearMileageValue();
      this.clearMileageValueUnloading(0);
      this.isOpenPayment = true; //默认支付方式都可选择
      this.goosnameList = []; //商品list清空
      if (this.ifUnloading == '02') {
        this.separateUnloadingShow = false;
      } else {
        this.separateUnloadingShow = true;
        this.goodsDeliveryCode = '#02';
        this.form.deliveryMethod = '02';
      }
    },
    handleSearch(value) {
      ...
      findMemberListByCustomer({
        ...
        this.selectList = [];
        if (res.data.results.memberList.length == 0) {
          this.selectList = [];
        }
        if (res.data.results.memberList) {
          this.selectList = res.data.results.memberList;
        }
      });
    },
    handleChangecustomerName(value) {
      ...
      this.form.buyerFirmName = value.memberCompanyName;
      this.form.settlementUnit = value.factoryCompanyName;
      this.form.memberId = value.memberId;
      this.form.buyerId = value.memberCustomerId;
      this.form.buyerName = value.memberCustomerName;
      this.form.buyerFirmId = value.memberOrgId;
      this.form.sellerId = value.factoryCustomerId;
      this.form.sellerFirmName = value.factoryCompanyName;
      this.form.sellerName = value.factoryCustomerName;
      this.form.sellerFirmId = value.factoryOrgId;

      this.memberIdCn = value.memberId; //memberId
      this.memberId = value.memberCustomerId; //买家id
      this.factoryId = value.factoryCustomerId; //厂家ID
      //【客户名称】更换客户，清空用气场站、清空专享价、清空运价
      this.form.stationIdarr = [];
      this.form.stationId = '';
      this.cleraprice(); //清空价格
      this.fxFeeColumns = [];
      this.form.mileageval = [];
      this.fndMemberStation();
      this.isPayment(value.factoryCustomerId);
    },
    isPayment(val) {
      //判断是否开通在线支付
      isOpenPayment({
        ...
        this.isOpenPayment = res.data.results.isOpenPayment;
        if (!this.isOpenPayment) {
          //如果没有开通在线支付则默认线下支付
          this.form.supportPayMethods = '02';
        }
      });
    },
    changeGoodname(value) {
      //选择商品
      //移除表单效验结果
      this.$refs.ruleForm.clearValidate('gasSourcesName');
      this.$refs.ruleForm.clearValidate('goodsQuantity');
      //【商品】更换商品，清空用气场站、重新渲染交货方式、清空专享价、清空运价
      this.form.stationIdarr = [];
      this.form.stationId = '';
      ...
      this.goodsList.forEach(item => {
        if (value == item.goodsInstId) {
          //渲染当前选中的商品
          this.goosnameList = item;
          //判断库存类型
          this.type = item.quantityType === '0' ? 2 : 1;
          //交货方式
          if (this.ifUnloading == '01') {
            //如果该商品只支持自提需要提示
            if (item.goodsDeliveryCode == '01#' || item.goodsDeliveryCode == '01') {
              this.$message.error('该商品只支持自提', 1);
              this.form.goodsName = '';
              this.goosnameList = '';
            }
            this.goodsDeliveryCode = '#02';
            this.form.deliveryMethod = '02';
            this.deliveryMethodShow = true;
          } else this.goodsDeliveryCode = item.goodsDeliveryCode;
          if (this.goosnameList) {
            this.form.goodsInstId = item.goodsInstId;
            //气源地
            this.form.gasSourcesName = item.gasSourcesName;
            //如果是配送的情况
            //押车标准
            this.form.stdDelay = item.contractDeal;
            //押车处理
            this.form.delayPrice = item.neededDeal;
            //气差范围
            this.form.gasheadStdRange = item.poorGasHead;
            //气差处理
            this.form.gasheadStdGap = item.oilHeadDeal;
            //购买数量
            this.form.goodsQuantity = item.transportStandard;
            //是否包含运费
            this.form.ifcontainsFreight = item.ifcontainsFreight;
            //分卸的购买数量
            this.form.DeliveryTerm[0].goodsQuantity = item.transportStandard / 2;
            //判断付款方式
            if (item.disburseType == '02') {
              //到付
              this.disburseType = true;
              this.form.payMethod = '02';
            } else {
              //预付
              this.disburseType = false;
              this.form.payMethod = '01';
            }
          } else {
            this.form.goodsInstId = '';
            this.form.gasSourcesName = '';
            this.form.stdDelay = '';
            this.form.delayPrice = '';
            this.form.gasheadStdRange = '';
            this.form.gasheadStdGap = '';
            this.form.DeliveryTerm[0].goodsQuantity = 11;
            this.disburseType = false;
            this.form.payMethod = '01';
          }
          //查询用气场站
    ...
    async dropdownVisibleChangeOpen(val, itemData) {
      let length1 = 0;
      if (this.separateUnloadingShow) {
        length1 = itemData.stationIdarr.length;
      } else {
        length1 = this.form.stationIdarr.length;
      }
      // 退出弹窗或完成选择，指向对应的省份及区县
      if (val) {
        // 增加API changeOnSelect后每次点击都会触发，增加判断减少计算次数
        if (length1 === 0 || length1 === 3) {
          //用气场站下拉展开
          await this.fndMemberStation();
          // 将全部项添加进数据內
          const content = { id: 'allData', text: '全部' };
          const content1 = { id: 'allData1', text: '全部' };
          const list = [];
          const list1 = [];
          this.memberStationList.forEach(item => {
            const content2 = { id: 'allData1', text: '全部' };
            const list2 = [];
            list.push(...item.children);
            item.children.forEach(item1 => {
              list2.push(...item1.children);
              list1.push(...item1.children);
            });
            content2.children = list2;
            item.children.unshift(content2);
          });
          content1.children = list1;
          list.unshift(content1);
          content.children = list;
          this.memberStationList.unshift(content);

          // 触发下拉框展示全部三联
          if (length1 === 0) {
            if (this.separateUnloadingShow) {
              itemData.stationIdarr = ['allData', 'allData1'];
            } else {
              this.form.stationIdarr = ['allData', 'allData1'];
            }
          }
        }
      } else {
        const stationId = this.separateUnloadingShow
          ? itemData.stationIdarr
          : this.form.stationIdarr;
        // 退出弹窗或完成选择，指向对应的省份及区县
        if (length1 < 3) {
          if (this.separateUnloadingShow) {
        ...
        } else if (stationId.includes('allData1')) {
          const tab = stationId[2];
          this.memberStationList.forEach(item => {
            item.children.forEach(items => {
              items.children.forEach(itemss => {
                if (itemss.id == tab) {
                  if (this.separateUnloadingShow) {
                    itemData.stationIdarr[0] = item.id;
                    itemData.stationIdarr[1] = items.id;
                  } else {
                    this.form.stationIdarr[0] = item.id;
                    this.form.stationIdarr[1] = items.id;
                  }
                }
              });
            });
          });
        } else if (stationId.includes('allData')) {
          const tab = stationId[1];
          this.memberStationList.forEach(item => {
            item.children.forEach(items => {
              if (items.id == tab) {
                if (this.separateUnloadingShow) {
    ...
    async fndMemberStation() {
      if (
        this.memberId &&
        this.goosnameList &&
        this.goosnameList.salesstatus &&
        this.goosnameList.salesstatus == '07'
      ) {
        await findMemberStationInfo({
          stationSearch: {
            sourceId: this.goosnameList ? this.goosnameList.gasSourcesId : '',
            buyerCustomerId: this.memberId,
            goodsRestrictedAreaCode:
              this.form.deliveryMethod == '01'
                ? this.goosnameList.goodsRestrictedAreaCodeZT
                : this.form.deliveryMethod == '02'
                ? this.goosnameList.goodsRestrictedAreaCodePS
                : '',
            stationSaleScope: this.goosnameList.stationSaleScope,
            version: 'v2',
          },
        }).then(res => {
          this.memberStationList = res.data.results.memberStationList;
        });
      } else {
        //查询用气场站
        await findMemberStationByCustomerIdAndSource({
          version: 'v2',
          ...
          goodsRestrictedAreaCode:
            this.form.deliveryMethod == '01'
              ...
              : '',
          factoryType: '01',
          deliveryMode: this.form.deliveryMethod,
          goodsInstId: this.goosnameList.goodsInstId,
          memberCustomerId: this.memberList.memberCustomerId,
          type: '20F',
        }).then(res => {
          this.memberStationList = res.data.results.memberStationList;
        });
    ...
    cascaderChangeBefore(val) {
      // 增加API changeOnSelect后每次点击都会触发，所以判断是否真选择完成
      if (val.length === 3) {
        // 选择完成
        this.cascaderChange(val);
      } else if (this.form.stationIdarr.length === 1 && this.form.stationIdarr[0] !== 'allData') {
        this.form.stationIdarr = [val[0], 'allData1'];
      }
    },
    cascaderChange(val) {
      //用气场站选中
      //【用气场站】更换用气场站，重置专享价、重置运价
      this.cleraprice(); //清空价格
      this.form.transportMileage = '';
      this.form.stationId = '';
      this.stationInfo = [];
      ...
      if (
        this.form.deliveryMethod == '01' &&
        this.goosnameList.stationRequired == 0 &&
        val.length == 0
      ) {
        this.priceListinit();
      }
      this.memberStationList.forEach(datas => {
        if (val[0] == datas.id) {
          datas.children.forEach(downtown => {
            if (val[1] == downtown.id) {
              downtown.children.forEach(tabs => {
                if (val[2] == tabs.id) {
                  this.stationInfo = tabs.stationInfo;
                  const stationPerson =
                    (tabs.stationInfo.stationPerson && tabs.stationInfo.stationTel) ||
                    (tabs.stationInfo.stationPerson && tabs.stationInfo.reason)
                      ? tabs.stationInfo.stationPerson
                      : tabs.stationInfo.stationPerson
                      ? tabs.stationInfo.stationPerson
                      : '';
                  const stationTel =
                    tabs.stationInfo.stationTel &&
                    tabs.stationInfo.reason &&
                    tabs.stationInfo.reason != ''
                      ? tabs.stationInfo.stationTel
                      : tabs.stationInfo.stationTel
                      ? tabs.stationInfo.stationTel
                      : '';
                  const reason = tabs.stationInfo.reason ? tabs.stationInfo.reason : '';
                  this.$set(this.form.DeliveryTerm[0], 'contact', stationPerson);
                  this.$set(this.form.DeliveryTerm[0], 'contactTel', stationTel);
                  this.$set(this.form.DeliveryTerm[0], 'remark', reason);
                }
              });
            }
          });
    ...
    termChangeBefore(val, index, event) {
      // 增加API changeOnSelect后每次点击都会触发，所以判断是否真选择完成
      if (val.stationIdarr.length === 3) {
        // 选择完成
        this.termChange(val, index);
      } else if (val.stationIdarr.length === 1 && val.stationIdarr[0] !== 'allData') {
        val.stationIdarr = [event[0], 'allData1'];
      }
    },
    async termChange(val, index) {
      this.clearMileageValueUnloading(index);
      //分卸的用气场站选择
      this.memberStationList.forEach(datas => {
        if (val.stationIdarr[0] == datas.id) {
          datas.children.forEach(downtown => {
            if (val.stationIdarr[1] == downtown.id) {
              datas.children.forEach(tabs => {
                if (val.stationIdarr[2] == tabs.id) {
                  this.stationInfo = tabs.stationInfo;
                  const stationPerson =
                    (tabs.stationInfo.stationPerson && tabs.stationInfo.stationTel) ||
                    (tabs.stationInfo.stationPerson && tabs.stationInfo.reason)
                  ...
                  const stationTel =
                    ...
                    tabs.stationInfo.reason != ''
                      ...
                      : '';
                  const reason = tabs.stationInfo.reason ? tabs.stationInfo.reason : '';
                  this.$set(this.form.DeliveryTerm[index], 'contact', stationPerson);
                  this.$set(this.form.DeliveryTerm[index], 'contactTel', stationTel);
                  this.$set(this.form.DeliveryTerm[index], 'remark', reason);
                  //场站
                  this.form.DeliveryTerm[index].stationname = tabs.text;
                  this.$forceUpdate();
                }
              });
            }
      ...
      this.form.DeliveryTerm[index].stationId = val.stationIdarr[1];
      //判断查询价格用气场站，商品，客户名称
      if (this.goosnameList && this.form.DeliveryTerm[index].stationId && this.memberId) {
        await this.priceListinit(true, index);
      }
      console.log(this.form.DeliveryTerm);
      //判断是否查询运费 商品，用气场站，计划到站时间（交货时间）配送
      if (
        // this.goosnameList.ifcontainsFreight == 'false' &&
        this.form.DeliveryTerm[index].stationId &&
        this.form.DeliveryTerm[index].deliveryTime
        // this.form.DeliveryTerm[index].deliveryTime &&
        // this.form.deliveryMethod == '02'
      ) {
        this.getFareList(index);
      }
      this.fxFeeColumns = [];
      this.form.mileageval = '';
      this.form.unloadCostCn = '';
      this.form.fxfeeprise = 0;
      //重新计算价格
      this.pricenew();
      //判断是否查询分卸里程
    ...
    inputChange() {
      //输入购买数量
      if (this.goosnameList.transportStandard) {
        const transportStandard = this.goosnameList.transportStandard;
        //goodsQuantity   挂牌总量（如果是不限，这个值是空）
        const max = Number(this.goosnameList.goodsQuantity);
        const min = Number(this.goosnameList.minPurchase);
        const num = min
          ? this.form.goodsQuantity / min
          : this.form.goodsQuantity / transportStandard;
        if (this.form.goodsQuantity == 0) {
          this.form.goodsQuantity = transportStandard;
          return;
        }
        if (!/(^[1-9]\d*$)/.test(num)) {
          this.form.goodsQuantity = Math.ceil(num) * transportStandard;
        }
        if (max) {
          if (this.form.goodsQuantity > max) {
            this.form.goodsQuantity = max;
            this.$message.warning('数量不能大于【' + max + '】', 2);
            return;
          }
        }
        if (min) {
          if (this.form.goodsQuantity < min) {
            this.form.goodsQuantity = min;
            this.$message.warning('数量不能小于【' + min + '】', 2);
            return;
          }
        }
      } else {
        this.$message.warning('请选择商品', 2);
        this.form.goodsQuantity = '';
      }
      this.form.settlementAmount = this.$accMul(this.price, this.form.goodsQuantity).toFixed(2);
    },
    handlePrev() {
      //购买数量减号点击
      //购买数量不能少于运输标准
      if (this.form.goodsQuantity && this.goosnameList.transportStandard) {
        const vals = Number(this.goosnameList.transportStandard); //运输标准
        const min = Number(this.goosnameList.minPurchase);
        let tcdata = 0;
        if (min) {
          //minPurchase最小下单量 transportStandard  运输标准（如果最小下单量为空，这个可以作为最小下单量）
          tcdata = Number(this.form.goodsQuantity) - min;
        } else tcdata = Number(this.form.goodsQuantity) - vals;

        if (tcdata < vals) {
          this.form.goodsQuantity = vals;
          this.$message.warning('数量不能少于【' + vals + '】', 2);
        } else this.form.goodsQuantity = tcdata;
      }
      this.form.settlementAmount = this.$accMul(this.price, this.form.goodsQuantity).toFixed(2);
    },
    handleNext() {
      //购买数量加号点击
      //goodsQuantity   挂牌总量（如果是不限，这个值是空）
      const max = this.goosnameList.goodsQuantity;
      if (this.form.goodsQuantity && this.goosnameList.transportStandard) {
        const vals = Number(this.goosnameList.transportStandard);
        const tcdata = Number(this.form.goodsQuantity) + vals;
        this.form.goodsQuantity = tcdata;
        if (max) {
          if (tcdata > max) {
            this.form.goodsQuantity = max;
            this.$message.warning('数量不能大于【' + max + '】', 2);
    ...
    pickerChange() {
      //计划到站时间选择日期
      const datea = this.$moment(this.form.planTime).format('YYYY-MM-DD');
      const timea = this.form.suppDateT
        ? this.$moment(this.form.suppDateT).format('HH:mm')
        : '00:00';
      if (this.form.planTime) {
        if (this.form.suppDateT) {
          this.form.suppDate = datea + ' ' + timea;
        }
        this.planTimedis = false;
      } else {
        this.planTimedis = true;
        //时间清空
        this.form.suppDateT = '';
      ...
      //判断是否重新计算价格
      if (this.goosnameList && this.form.stationId && this.form.deliveryMethod == '02') {
        this.Recalculation();
      }
    },
    pickerTimeChange() {
      //计划到站选择时间
      const datea = this.$moment(this.form.planTime).format('YYYY-MM-DD');
      const timea = this.$moment(this.form.suppDateT).format('HH:mm');
      if (this.form.suppDateT) {
        this.form.suppDate = datea + ' ' + timea;
      } else {
        this.form.suppDate = '';
    ...
    oherblur() {
      //其他优惠输入
      //成交价 = 专享价+运费-其他优惠
      this.price = this.$accSub(
        this.$accAdd(this.agreePromotion.promotionDelList[0].discountPrice, this.form.fare),
        this.form.otherDiscount
      );
      this.form.settlementAmount = this.$accMul(this.price, this.form.goodsQuantity).toFixed(2);
    },
    otherinput() {
      //其他优惠输入中
      if (this.form.otherDiscount.length > 4 || !/(^[0-9]\d*$)/.test(this.form.otherDiscount)) {
        this.$message.warning('其他优惠请输入1-4位正整数', 2);
        this.form.otherDiscount = '';
      }
    },
    termoherblur(index) {
      //分卸的其他优惠输入
      if (this.form.DeliveryTerm[index].stationId) {
        //成交价 = 专享价+运费-其他优惠+分卸费
        this.price =
          this.$accSub(
            Number(this.form.DeliveryTerm[index].price) +
              Number(this.form.DeliveryTerm[index].fare),
            this.form.DeliveryTerm[index].otherDiscount
          ) + Number(this.form.fxfeeprise);
        //小计 小计=成交价*量
        this.form.DeliveryTerm[index].cifcost = this.$accMul(
          this.price,
          this.form.DeliveryTerm[index].goodsQuantity
        );
        //成交额 成交额=小计的和
        let total = 0;
        this.form.DeliveryTerm.forEach(item => {
          total = total + item.cifcost;
        });
        this.form.cifcostTotal = total.toFixed(2);
      } else {
        this.$message.warning('请选择用气场站', 2);
        this.form.DeliveryTerm[index].otherDiscount = 0;
      }
    },
    otherdisinput(index) {
      //其他优惠输入中判断
      if (
        this.form.DeliveryTerm[index].otherDiscount.length > 4 ||
        !/(^[0-9]\d*$)/.test(this.form.DeliveryTerm[index].otherDiscount)
      ) {
        this.$message.warning('其他优惠请输入1-4位正整数', 2);
        this.form.DeliveryTerm[index].otherDiscount = '';
      }
    },
    goosquantinput(index) {
      //分卸的购买数量输入
      //成交价 = 专享价+运费-其他优惠+分卸费
      this.price =
        this.$accSub(
          Number(this.form.DeliveryTerm[index].price) + Number(this.form.DeliveryTerm[index].fare),
          this.form.DeliveryTerm[index].otherDiscount
        ) + Number(this.form.fxfeeprise);
      //小计 小计=成交价*量
      this.form.DeliveryTerm[index].cifcost = this.$accMul(
    ...
    getFareList(index) {
      if (!this.ifcost()) return;
      //查询运费接口
      // let suppDates = new Date();
      // suppDates = this.$moment(suppDates).format('YYYY-MM-DD HH:mm:ss');
      // const datas = {
      //   sourceId: this.goosnameList.gasSourcesId,
      //   stationId:
      //     this.ifUnloading == '02' ? this.form.stationId : this.form.DeliveryTerm[index].stationId,
      //   suppDate: suppDates,
      //   noMessage: true,
      // };
      // getFareList(datas)
      ...
      if (
        this.goosnameList.salesstatus == '07' &&
        this.goosnameList.quantityType !== '1' &&
        this.form.deliveryMethod.includes('01')
      ) {
        this.priceListinit();
        return;
      }
      if (
        this.goosnameList.salesstatus == '07' &&
        this.goosnameList.quantityType !== '1' &&
        this.form.deliveryMethod.includes('02') &&
        !(this.goosnameList.ifcontainsFreight == 'false')
      ) {
        this.priceListinit();
        return;
      ...
      const datas = {
        ...
        goodInstId: this.form.goodsName,
        stationList:
          this.ifUnloading == '02' || this.form.deliveryMethod.includes('01')
            ? [
                {
                  arriveDate: this.$moment(this.form.suppDate).format('YYYY-MM-DD HH:mm:ss'),
                  expectArriveDate: this.$moment(new Date()).format('YYYY-MM-DD HH:mm:ss'),
                  stationId: this.form.stationId,
                },
              ]
            : this.form.DeliveryTerm.map(item => {
                return {
                  arriveDate: this.$moment(item.deliveryTime).format('YYYY-MM-DD HH:mm:ss'),
                  expectArriveDate: this.$moment(new Date()).format('YYYY-MM-DD HH:mm:ss'),
      ...
      // this.form.fare = 0;
      // this.form.transportMileage = '';
      // this.form.DeliveryTerm.forEach(cl => {
      //   cl.farenew = 0;
      //   cl.fare = 0;
      //   cl.transportMileage = '';
      // });
      // this.clearMileageValue(index);
      getFareByYuntu(datas)
        .then(res => {
          //如果自提 和 包含运费获取里程
          if (
            !(this.goosnameList.ifcontainsFreight == 'false') ||
            ...
            //不分卸和自提
            if (this.ifUnloading == '02' || this.form.deliveryMethod == '01') {
              this.form.transportMileage = res.data.results.data.transportMileage;
              this.form.simpleFare = res.data.results.data.freightRate;
              this.priceListinit();
              Object.assign(this.form, res.data.results.data);
            } else {
              this.form.DeliveryTerm.map((item, indexs) => {
                item.transportMileage = res.data.results.data.transportMileage;
                item.simpleFare = res.data.results.data.freightRate;
                this.priceListinit(true, indexs);
                // return Object.assign(item, res.data.results.data);
                Object.assign(item, res.data.results.data);
              });
              this.form.DeliveryTerm = JSON.parse(JSON.stringify(this.form.DeliveryTerm));
            }
          } else {
            if (this.ifUnloading == '02') {
              this.form.fare = res.data.results.data.freightRate;
              this.form.simpleFare = res.data.results.data.freightRate;
              this.form.transportMileage = res.data.results.data.transportMileage;
              this.priceListinit();
              Object.assign(this.form, res.data.results.data);
            } else {
              // this.priceListinit(true, index);
              // this.form.DeliveryTerm[index].farenew = res.data.results.data.freightRate;
              // this.form.DeliveryTerm[index].fare = res.data.results.data.freightRate;
              this.form.DeliveryTerm.map((item, indexs) => {
                item.farenew = res.data.results.data.freightRate;
                item.fare = res.data.results.data.freightRate;
                item.simpleFare = res.data.results.data.freightRate;
                item.transportMileage = res.data.results.data.transportMileage;
                this.priceListinit(true, indexs);
                // return Object.assign(item, res.data.results.data);
            ...
            //   null,
            //   this.form.DeliveryTerm.map(function (o) {
            //     return o.farenew;
            //   })
            // );
            // this.form.DeliveryTerm.forEach(item => {
            ...
            //     item.fare = '';
            //   } else item.fare = maxnum;
            // });
            this.pricenew();
          }
        })
        .catch(err => {
          this.$message.warning(err.response.data.describe, 2);
          if (!this.separateUnloadingShow) {
            this.form.stationIdarr = [];
            this.form.stationId = '';
          } else {
            this.form.DeliveryTerm[index].stationIdarr = [];
            this.form.DeliveryTerm[index].stationId = '';
          }
          if (this.ifUnloading == '02') {
            this.form.fare = 0;
            ...
            // this.priceListinit(true, index);
            // this.form.DeliveryTerm[index].farenew = 0;
            // this.form.DeliveryTerm[index].fare = '';
            this.form.DeliveryTerm.forEach((item, indexs) => {
              this.priceListinit(true, indexs);
              item.farenew = 0;
              item.fare = 0;
              item.transportMileage = '';
            });
            // 最大值运费
            // let maxnum = 0;
            // maxnum = Math.max.apply(
    ...
    clearMileageValue() {
      this.form.fare = 0;
      this.form.simpleFare = 0; //保存假字段
      this.form.transportMileage = '';
      this.form.freightRate = 0;
      this.form.fcLogId = '';
      this.form.regularFareUnit = '';
      this.form.regularFarePrice = '';
      this.form.regularMileage = '';
      this.form.regularMileageSource = '';
      this.form.unloadFarePrice = '';
      this.form.unloadMileage = '';
      this.form.unloadMileageSource = '';
      this.form.unloadUnit = '';
    },
    //运费接口清空数据
    clearMileageValueUnloading(index) {
      this.form.DeliveryTerm[index].farenew = 0;
      this.form.DeliveryTerm[index].fare = 0;
      this.form.DeliveryTerm[index].simpleFare = 0; //保存假字段
      this.form.DeliveryTerm[index].transportMileage = '';
      this.form.DeliveryTerm[index].freightRate = 0;
      this.form.DeliveryTerm[index].fcLogId = '';
      this.form.DeliveryTerm[index].regularFareUnit = '';
      this.form.DeliveryTerm[index].regularFarePrice = '';
      this.form.DeliveryTerm[index].regularMileage = '';
      this.form.DeliveryTerm[index].regularMileageSource = '';
      this.form.DeliveryTerm[index].unloadFarePrice = '';
      this.form.DeliveryTerm[index].unloadMileage = '';
      this.form.DeliveryTerm[index].unloadMileageSource = '';
      this.form.DeliveryTerm[index].unloadUnit = '';
    },

    async priceListinit(val, index) {
      //价格信息接口
      const datas = {
        //参数
        agreeDelListStr: [
          {
            goodsId: this.goosnameList.goodsInstId, //商品ID
            stationId: val ? this.form.DeliveryTerm[index]?.stationId : this.form.stationId, //场站ID
            supplyNum: val ? this.form.DeliveryTerm[index]?.goodsQuantity : this.form.goodsQuantity, //购买数量
            otherDiscount: '',
            deliveryTime: val ? this.form.DeliveryTerm[index]?.planTime : this.form.suppDate, //计划到站时间
          },
        ],
        goodsGuidingPrice:
          this.goosnameList?.priceSetting == '03'
            ? ''
            : this.$accDiv(this.goosnameList.goodsGuidingPrice, 100), //挂牌价
        factoryId: '', //厂家ID
        deliveryMode: this.form.deliveryMethod, //配送方式
        ifUnloading: this.ifUnloading, //是否分卸
        buyCustomerId: this.memberId, //买家ID
        orderTime: this.$moment(new Date()).format('YYYY-MM-DD HH:mm:ss'), //下单时间(当前时间)
      };

      await calcAgreePromotion(datas)
        .then(res => {
          this.$set(this.disabledList, index, '1');
          if (this.disabledList.indexOf('2') != -1) {
            this.butdisabled = true;
          } else {
            this.butdisabled = false;
          }
          this.agreePromotion = res.data.results.agreePromotion;
          console.log(this.agreePromotion);
          if (this.goosnameList.freightShow === 0) {
            console.log(this.ifUnloading);
            if (this.ifUnloading == '02') {
              this.agreePromotion.hasFareDiscountPrice = this.$accAdd(
                this.agreePromotion.promotionDelList[0].discountPrice,
                this.form.fare
              );
              this.agreePromotion.hasFareGoodsPrice = this.$accAdd(
                this.agreePromotion.promotionDelList[0].goodsPrice,
                this.form.fare
              );
            } else {
              this.$nextTick(() => {
                this.form.DeliveryTerm.forEach(item => {
                  item.hasFareDiscountPrice = item.price + item.fare;
                });
              });
            }
          } else {
            if (this.ifUnloading == '02') {
              this.agreePromotion.hasFareDiscountPrice =
                this.agreePromotion.promotionDelList[0].discountPrice;
              this.agreePromotion.hasFareGoodsPrice =
                this.agreePromotion.promotionDelList[0].goodsPrice;
            } else {
              this.$nextTick(() => {
                this.form.DeliveryTerm.forEach(item => {
                  item.hasFareDiscountPrice = item.price;
                });
              });
            }
          }
          //判断优惠政策
          let promotionLineList = [];
          if (this.agreePromotion.promotionDelList[0].promotionLineList) {
            promotionLineList = this.agreePromotion.promotionDelList[0].promotionLineList;
            promotionLineList.forEach(item => {
              item.name = item.discountName;
              item.value = item.discountAmount;
            });
          }
          this.offerDetailsData = [
            {
              name: '挂牌价',
              value: this.agreePromotion.promotionDelList[0].goodsPrice,
              id: Math.ceil(Math.random() * 100000),
            },
            {
              name: '购气专享价',
              value: this.agreePromotion.promotionDelList[0].discountPrice,
              id: Math.ceil(Math.random() * 100000),
            },
            {
              name: '运价',
              value: val ? this.form.DeliveryTerm[index].fare : this.form.fare,
              id: Math.ceil(Math.random() * 100000),
            },
            // {
            //   name: '分卸费',
            //   value: this.form.fxfeeprise,
            //   show: val,
            //   id: Math.ceil(Math.random() * 100000),
            // },
            {
              name: '其它优惠',
              value: val ? this.form.DeliveryTerm[index].otherDiscount : this.form.otherDiscount,
              id: Math.ceil(Math.random() * 100000),
            },
          ];
          if (val) {
            //分卸
            if (this.goosnameList.freightShow === 0) {
              this.offerDetailsData[0].value = this.$accAdd(
                this.offerDetailsData[0].value,
                Number(this.form.DeliveryTerm[index].fare)
              );
              this.offerDetailsData[1].value = this.$accAdd(
                this.offerDetailsData[1].value,
                Number(this.form.DeliveryTerm[index].fare)
              );
            }
            this.form.DeliveryTerm[index].agreePromotion = res.data.results.agreePromotion;
            this.form.DeliveryTerm[index].offerDetailsData =
              this.offerDetailsData.concat(promotionLineList);
            //专享价
            this.form.DeliveryTerm[index].price =
              this.agreePromotion.promotionDelList[0].discountPrice;
            //成交价 = 专享价+运费+分卸费-其他优惠
            this.price =
              Number(this.form.DeliveryTerm[index].price) +
              Number(this.form.DeliveryTerm[index].fare) +
              Number(this.form.fxfeeprise) -
              Number(this.form.DeliveryTerm[index].otherDiscount);
            //小计 小计=成交价*量
            this.form.DeliveryTerm[index].cifcost = this.$accMul(
              this.price,
              this.form.DeliveryTerm[index].goodsQuantity
            ...
            this.form.DeliveryTerm[0].agreePromotion = res.data.results.agreePromotion;
            this.offerDetailsData = this.offerDetailsData.concat(promotionLineList);
            //成交价 = 专享价+运费-其他优惠
            this.price =
              Number(this.agreePromotion.promotionDelList[0].discountPrice) +
              Number(this.form.fare) -
              Number(this.form.otherDiscount);
            this.form.settlementAmount = this.$accMul(this.price, this.form.goodsQuantity).toFixed(
              2
            );
          }
          this.$forceUpdate();
        ...
        .catch(err => {
          //该场站所属的销售区域暂无价格，请重新选择，清空场站
          if (err?.response?.data?.state === '200001') {
            if (!this.separateUnloadingShow) {
              this.form.stationIdarr = [];
            } else {
              this.form.DeliveryTerm[index].stationIdarr = [];
          ...
          this.$set(this.disabledList, index, '2');
          if (this.disabledList.indexOf('2') != -1) {
            this.butdisabled = true;
          } else {
            this.butdisabled = false;
          }
          this.form.DeliveryTerm[index].price = 0;
          this.form.DeliveryTerm[index].fare = 0;
          this.form.DeliveryTerm[index].otherDiscount = 0;
          this.offerDetailsData = [
            {
          ...
          this.form.DeliveryTerm[index].offerDetailsData = this.offerDetailsData;
        });
    },
    termpickerChange(index) {
      //分卸的交货日期选择
      if (this.form.DeliveryTerm[index].planTime) {
        const datea = this.$moment(this.form.DeliveryTerm[index].planTime).format('YYYY-MM-DD');
        const timea = this.form.DeliveryTerm[index].suppDateT
          ? this.$moment(this.form.DeliveryTerm[index].suppDateT).format('HH:mm')
          : '00:00';
        if (this.form.DeliveryTerm[index].suppDateT) {
          this.form.DeliveryTerm[index].deliveryTime = datea + ' ' + timea;
        } else {
          this.form.DeliveryTerm[index].deliveryTime = '';
        }
        this.form.DeliveryTerm[index].planTimedis = false;
        this.checkTime(index);
      } else {
        this.form.DeliveryTerm[index].planTimedis = true;
        this.form.DeliveryTerm[index].suppDateT = '';
        this.form.DeliveryTerm[index].deliveryTime = '';
      }
      //判断是否查询分卸里程
      // this.ifcost();
      ...
      if (
        ...
        this.Recalculation(true, index);
      }
    },
    termpickerTimeChange(index) {
      //分卸的交货时间选择
      const datea = this.$moment(this.form.DeliveryTerm[index].planTime).format('YYYY-MM-DD');
      const timea = this.$moment(this.form.DeliveryTerm[index].suppDateT).format('HH:mm');
      if (this.form.DeliveryTerm[index].suppDateT) {
        this.form.DeliveryTerm[index].deliveryTime = datea + ' ' + timea;
      } else {
        this.form.DeliveryTerm[index].deliveryTime = '';
    ...
    checkTime(index) {
      //分卸状态配送期望到货时间不能相同+第X个分卸点时间需大于第X个分卸点时间
      this.form.DeliveryTerm.forEach((item, tableIndex) => {
        if (
          index != tableIndex &&
          item.planTime &&
          item.suppDateT &&
          item.deliveryTime == this.form.DeliveryTerm[index].deliveryTime
        ) {
          this.form.DeliveryTerm[index].planTime = '';
          this.form.DeliveryTerm[index].suppDateT = '';
          this.form.DeliveryTerm[index].deliveryTime = '';
          this.form.DeliveryTerm[index].planTimedis = true;
          this.$message.warning('分卸场站的交货时间不能相同，请重新选择', 2);
        }
        //第X个分卸点时间需大于第X个分卸点时间
        if (index > 0) {
          if (tableIndex == index - 1) {
            const data1 = this.$moment(item.planTime).format('YYYY-MM-DD');
            const data2 = this.$moment(this.form.DeliveryTerm[index].planTime).format('YYYY-MM-DD');
            const time1 = this.$moment(item.suppDateT).format('HH:mm');
            const time2 = this.$moment(this.form.DeliveryTerm[index].suppDateT).format('HH:mm');
            if (data1 > data2 || (data1 == data2 && time1 > time2)) {
              this.form.DeliveryTerm[index].planTime = '';
              this.form.DeliveryTerm[index].suppDateT = '';
              this.form.DeliveryTerm[index].deliveryTime = '';
              this.form.DeliveryTerm[index].planTimedis = true;
              this.$message.warning(
                `第${index + 1}个分卸点时间需大于第${tableIndex + 1}个分卸点时间`,
                2
              );
            }
          }
    ...
    handleDelete(item) {
      //删除一条交货需求
      this.form.DeliveryTerm.forEach((element, index) => {
        if (element.id == item.id) {
          this.form.DeliveryTerm.splice(index, 1);
        }
      });
      //清空分卸里程
      this.fxFeeColumns = [];
    ...
    handleAdd() {
      //新增一条交货需求
      if (this.form.DeliveryTerm.length < 10) {
        this.form.DeliveryTerm.push({
          id: Math.ceil(Math.random() * 100000),
          stationIdarr: '',
          planTime: '', //交货日期
          suppDateT: '', //交货时间
          ...
          contactTel: '',
          remark: '',
          goodsQuantity: this.goosnameList.transportStandard
            ? this.goosnameList.transportStandard / 2
            : 11, //购买数量
          price: '0.00', //价格
          fare: 0, //运价
          farenew: 0,
    ...
    onSubmit() {
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          ...
          const preData = JSON.parse(JSON.stringify(this.form.DeliveryTerm));
          if (this.ifUnloading == '01') {
            preData.forEach(res => {
              if (!res.stationId) {
                this.$message.warning('用气场站不能为空', 2);
                throw new Error('用气场站不能为空');
              }
            });
            // 以防万一出现时间组件bug，日期未选择的情况下没有清空时间
            preData.forEach(res => {
              if (!res.deliveryTime || !res.planTime || !res.suppDateT) {
                this.$message.warning('时间不能为空', 2);
          ...
          let station = 0,
            ...
            stationId = 0,
            suppDate = 0;
          promotionInfoLineList = this.agreePromotion.promotionDelList[0].promotionLineList;
          this.form.deliveryMode = this.form.deliveryMethod; //交货方式：01自提；02配送
          this.form.agreeType = '01'; //协议类型 01 :舟山现货协议
          //分卸非分卸分开判断
          preData.forEach((item, i) => {
            item.goodsInstId = this.goosnameList.goodsInstId; //商品id
            item.goodsName = this.goosnameList.goodsName; //商品名称
            item.stationId = this.ifUnloading == '02' ? this.form.stationId : item.stationId; //场站id
            item.stationName =
              this.ifUnloading == '02' ? this.stationInfo.stationName : item.stationName; //场站名称
            item.sourceId = this.goosnameList.gasSourcesId; //气源地ID
            item.sourceName = this.goosnameList.gasSourcesName; //气源地名称
            item.transportStandard = this.goosnameList.transportStandard;
            item.goodsId = this.goosnameList.goodsId;
            item.agreeSupplyNum =
              this.ifUnloading == '02' ? this.form.goodsQuantity : item.goodsQuantity; //协议量
            item.supplyNum =
              this.ifUnloading == '02' ? this.form.goodsQuantity : item.goodsQuantity; //协议量
            item.suppDate = this.ifUnloading == '02' ? this.form.suppDate : item.deliveryTime; //计划到站时间/计划提货时间
            item.cifcost =
              this.ifUnloading == '02'
                ? this.price
                : this.$accDiv(item.cifcost, item.goodsQuantity); //成交价
            item.cifcost = Number(item.cifcost) * 100;
            item.gasifRatio = this.goosnameList.gasSourcesRatio; //气化率
            item.heatValue = this.goosnameList.gasHeatValue; //热值
            item.liquitTemperature = this.goosnameList.gasLtempValue; //液温
            item.sourcesType = this.goosnameList.gasSourcesType; //气质类型
            item.listingPrice = this.$accMul(
              item.agreePromotion.promotionDelList[0].goodsPrice,
              100
            ); //挂牌价（单位：分）
            if (!this.numberVerification(item.contactTel)) {
              throw new Error();
            }
            item.exclusivePrice =
              this.ifUnloading == '02'
                ? this.$accMul(this.agreePromotion.promotionDelList[0].discountPrice, 100)
                : this.$accMul(item.price, 100); //专享价（单位：分）
            item.fare =
              this.ifUnloading == '02'
                ? this.$accMul(this.form.fare, 100)
                : item.fare
                ? this.$accMul(item.fare, 100)
                : ''; //运价（单位：分）
            item.transportMileage =
              this.ifUnloading == '02' || this.form.deliveryMethod == '01'
                ? this.form.transportMileage
                : item.transportMileage
                ? item.transportMileage
                : ''; //里程（单位：公里）
            item.otherDiscount =
              this.ifUnloading == '02'
                ? this.$accMul(this.form.otherDiscount, 100)
                : this.$accMul(item.otherDiscount, 100); //其它优惠（单位：分）
            item.deliveryTime = this.form.deliveryTime; //预计发车时间
            item.fxFee = this.ifUnloading == '02' ? 0 : this.form.fxfeeprise; //分卸金额元
            item.mileageId = this.ifUnloading == '02' ? '' : this.form.temporaryMileageId; //分卸id
            item.promotionInfoLineList = promotionInfoLineList;
            if (!item.stationId) stationId += stationId + 1;
            if (!item.suppDate) suppDate += suppDate + 1;
            if (this.ifUnloading == '01') {
              item.promotionInfoLineList = item.offerDetailsData.slice(5);
            }
            for (let j = i + 1; j < preData.length; j++) {
              if (preData[i].stationId === preData[j].stationId) {
                station = station + 1;
              }
              if (preData[i].suppDate === preData[j].suppDate) {
                suppdateNum = suppdateNum + 1;
              }
            }
            if (
            ...
            if (Number(item.supplyNum) / Number(this.goosnameList.transportStandard) <= 0.1)
              less = less + 1;
            currentSupplyNum += Number(item.supplyNum);
          });
          //分卸判断
          if (this.ifUnloading == '01') {
            if (stationId > 0) {
            ...
            if (preData.length == 1) {
              this.$message.warning('分卸时必须是多条明细整车', 2);
              return false;
            }
            if (less > 0) {
              this.$message.warning(`分卸单条明细必须大于0.1车，请调整数量`, 2);
              return false;
            }
            // 一条明细：数量为运输标准的一半；(向上取整)
            // 多条明细：所有明细的总数量等于运输标准(向上取整)
            if (
              preData.length == 1 &&
              Math.ceil(this.goosnameList.transportStandard / 2) != currentSupplyNum
            ) {
              this.$message.warning(
                `单点分卸数量需为${Math.ceil(this.goosnameList.transportStandard / 2)}吨`,
                2
              );
              return false;
            } else if (
              preData.length > 1 &&
              this.goosnameList.transportStandard != currentSupplyNum
            ) {
              this.$message.warning(
                `多点分卸数量之和需为${this.goosnameList.transportStandard}吨`,
                2
              );
              return false;
            }
            //多个交货需求的分卸站点不可一样
            if (station > 0) {
              this.$message.warning('分卸时场站不能相同', 2);
              return false;
            }
            if (suppdateNum > 0) {
              this.$message.warning('分卸时交货时间不能相同！', 2);
              return false;
            }
            if (this.form.deliveryMode == '02') {
              if (fare > 0) {
                this.$message.warning('配送时运费不能为空', 2);
                return false;
              }
            }
          } else {
            //非分卸判断
            if (this.form.suppDate == '') {
              this.$message.warning('计划到站时间请选择时分秒', 2);
              return false;
            }
            if (this.form.deliveryMode == '02') {
              if (fare > 0) {
          ...
          //如果是自提和包含运费 运费保存在simpleFare中
          if (
            !(this.goosnameList.ifcontainsFreight == 'false') ||
            this.form.deliveryMethod == '01'
          ) {
            preData.map(items => {
              items.freightSituation =
                this.ifUnloading == '02'
                  ? this.form.simpleFare
                  : items.simpleFare
                  ? items.simpleFare
                  : ''; //运价（单位：分）
              items.fare = 0;
          ...
          if (this.ifUnloading == '02' || this.form.deliveryMethod == '01') {
            preData.map(items => {
              items.fcLogId = this.form.fcLogId;
              items.regularFareUnit = this.form.regularFareUnit;
              items.regularFarePrice = this.form.regularFarePrice;
              items.regularMileage = this.form.regularMileage;
              items.regularMileageSource = this.form.regularMileageSource;
              items.unloadFarePrice = this.form.unloadFarePrice;
              items.unloadMileage = this.form.unloadMileage;
              items.unloadMileageSource = this.form.unloadMileageSource;
              items.unloadUnit = this.form.unloadUnit;
            });
          }
          //新需求都走摘单流程，后续需要调整回来再说
          this.handStockAlternate(preData);
          //如果是1则走摘单流程,不是则走以前的正常提交流程
          // if(this.goosnameList.quantityType==='1'){
          //   this.handStockAlternate(preData)
          // }else this.handleSubmit(preData)
        } else {
          console.log('error submit!!');
          return false;
        }
    ...
    handStockAlternate(preData) {
      //推单的弹窗
      this.showSpin = true;
      const params = JSON.parse(JSON.stringify(this.form));
      params.delayPrice = this.$accMul(params.delayPrice, 100);
      params.ifUnloading = this.ifUnloading;
      params.delListStr = preData;
      stock({ agreeInfoVo: params, noMessage: true })
        .then(res => {
          this.showSpin = false;
          this.agreeDelStockList = res.data.results.agreeInfoStock;
          this.alternateshow = true;
        })
        .catch(err => {
          this.$message.warning(
            err?.response?.data?.describe || '当前网络不通畅，请检查您的网络设置！'
          );
          this.showSpin = false;
        });
    },
    handleSubmit(preData) {
      //提交协议
      const params = JSON.parse(JSON.stringify(this.form));
      params.delayPrice = this.$accMul(params.delayPrice, 100);
      params.ifUnloading = this.ifUnloading;
      params.delListStr = preData;
      createEntrustAgree({ agreeInfoVo: params, noMessage: true })
        .then(res => {
          this.$message.success(res.data.describe, 2);
          this.$success({
            title: res.data.describe,
            okText: '确认',
            onOk: () => {
              window.location.href = this.location + 'new/entrustAgree/list';
            },
          });
        })
        .catch(err => {
    ...
    handleDetails(item, index) {
      // 优惠详情
      this.offerDetailsShow = true;
      if (item) {
        //如果是分卸
        this.offerDetailsData = this.form.DeliveryTerm[index].offerDetailsData;
        this.price = 0;
        this.offerDetailsData[2].value = Number(this.form.DeliveryTerm[index].fare);
        // this.offerDetailsData[3].value = this.form.fxfeeprise;
        this.offerDetailsData[3].value = Number(this.form.DeliveryTerm[index].otherDiscount);
        if (this.form.DeliveryTerm[index].stationId) {
          //成交价 = 专享价+运费+ 分卸费-其他优惠
          if (this.goosnameList.freightShow === 0) {
            this.price =
        ...
        //非分卸
        this.offerDetailsData[3].value = Number(this.form.otherDiscount);
        //成交价 = 专享价+运费-其他优惠
        this.price =
          Number(this.agreePromotion.promotionDelList[0].discountPrice) +
          Number(this.form.fare) -
      ...
      this.offerDetailsData = JSON.parse(JSON.stringify(this.offerDetailsData));
    },
    handleModelCancel() {
      //价格明细关闭
      this.offerDetailsShow = false;
    },
    ifcost() {
      //判断是否调用分卸里程接口
      let station = true;
      //判断分卸里程选择 必须要有商品，客户名称，分卸站点，交货时间
      if (this.ifUnloading == '02') {
        if (!this.form.stationId || !this.form.suppDate) {
          station = false;
        }
      } else {
        this.form.DeliveryTerm.forEach(item => {
      ...
      if (this.goosnameList && station && this.memberId) {
        return true;
        // this.getUnloadCost();
      }
      return false;
    ...
    getUnloadCost() {
      ...
      this.form.DeliveryTerm.forEach(item => {
        ...
        datas.push(stationarr);
      });
      const data = {
        memberId: this.memberIdCn,
        deliveryList: datas,
      };
      getUnloadCost({ data: data }).then(res => {
        this.fxFeeColumns = res.data.results.data.deliveryList;
        this.form.unloadPathStationIdName = res.data.results.data.unloadPathStationIdName;
        if (res.data.results.data.autoCalc) {
          this.form.mileageval = `${this.fxFeeColumns[0].unloadCostId}//${this.fxFeeColumns[0].unloadCostCn}//${this.fxFeeColumns[0].price}`;
          this.form.unloadCostCn = this.fxFeeColumns[0].unloadCostCn;
          this.form.temporaryMileageId = this.fxFeeColumns[0].unloadCostId;
          this.form.temporaryFxFee = this.fxFeeColumns[0].unloadCostCn;
          this.form.fxfeeprise = this.fxFeeColumns[0].price;
        }
      });
    },
    selectChange(value) {
      ...
      this.form.temporaryMileageId = arr[0];
      this.form.temporaryFxFee = arr[1];
      this.form.unloadCostCn = arr[1];
      this.form.fxfeeprise = arr[2];
      //重新计算价格
      this.pricenew();
    },
    remarkinput() {
    ...
    cleraprice() {
      //清空价格、优惠详情
      this.form.cifcostTotal = '0.00';
      this.price = '0.00';
      this.form.settlementAmount = '0.00';
      this.form.fare = 0;
      this.form.remark = '';
      this.agreePromotion.promotionDelList[0].discountPrice = 0;
      this.form.DeliveryTerm = [
        {
          id: Math.ceil(Math.random() * 100000),
          stationIdarr: '',
          planTime: '', //交货日期
    ...
    Recalculation(val, index) {
      ...
      if (this.agreePromotion.promotionDelList[0].promotionLineList) {
        if (val) {
          promotionLineList =
            this.form.DeliveryTerm[index].agreePromotion.promotionDelList[0].promotionLineList;
        } else promotionLineList = this.agreePromotion.promotionDelList[0].promotionLineList;
        promotionLineList.forEach(item => {
          item.name = item.discountName;
          item.value = item.discountAmount;
        });
      ...
      this.offerDetailsData = [
        {
          name: '挂牌价',
          value: val
            ? this.form.DeliveryTerm[index].agreePromotion.promotionDelList[0].goodsPrice
            : this.agreePromotion.promotionDelList[0].goodsPrice,
          id: Math.ceil(Math.random() * 100000),
        },
        {
          name: '购气专享价',
          value: val
            ? this.form.DeliveryTerm[index].agreePromotion.promotionDelList[0].discountPrice
            : this.agreePromotion.promotionDelList[0].discountPrice,
          id: Math.ceil(Math.random() * 100000),
        },
        {
    ...
    pricenew() {
      ...
      this.form.DeliveryTerm.forEach(item => {
        ...
        this.price =
          ...
          Number(item.fare) +
          Number(this.form.fxfeeprise) -
          Number(item.otherDiscount);
        //小计 小计=成交价*量
        // console.log(this.form.fxfeeprise+'---'+this.price+'----'+item.goodsQuantity)
        item.cifcost = this.$accMul(this.price, item.goodsQuantity);
        total = total + item.cifcost;
      });
      this.form.cifcostTotal = total.toFixed(2);
    },
...
<style src="./manufator.scss" lang="scss" scoped />
<style lang="scss" scoped>
.from-information-head {
  padding: 14px 0;
  margin-bottom: 15px;
  background: #f7f7f7;
  box-shadow: inset 0 -1px 0 0 #e5edf4;
}
...
## [ZNJY-4976](https://jira.enncloud.cn/browse/ZNJY-4976)
...
> features/1030-colourSystemUnify
>
> 005196
>
> 005195
...
```scss
// this.$confirm的弹窗图标
::v-deep .ant-modal-confirm-confirm .ant-modal-confirm-body > .anticon{
  color: #FF9200;
}
...
::v-deep {
  ...
  .ant-time-picker-input[disabled] {
    border-color: #c8c9cc66 !important;
    color: #c8c9cc;
...
![image-20230904162822007](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230904162822007.png)
...
涉及到灰色状态这个颜色统一改为：7D7E80；目前这个颜色太浅了，UI提供了新色值



还有tab的时候，待上架那些字鼠标上移需要改成0473ff蓝色主色
...
.ant-table-tbody {
  > tr .ant-table-row-hover:not(.ant-table-expanded-row, .ant-table-row-selected) > td,
  > tr:hover:not(.ant-table-expanded-row, .ant-table-row-selected) > td,
...
![image-20230921153856986](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230921153856986.png)
...
::v-deep .ant-modal-footer button:first-child {
    color: #0473ff;
    border: none;
    background-color: #f2f3f5;
  }	
...
> https://jira.enncloud.cn/browse/ZNJY-5860
>
> base：features/1116-addLabel

> 标签嵌入页面：
>
> 027A1 728C8 F042F 99564 1E7DA C0F70 E8
>
> B89C4DF61B27E067652E98930C7FDCCD
>
> B89C4DF61B27E067652E98930C7FDCCD
>
> findLoginUserInfo接口里面加一个字段返回
>
> 是否主账号为了判断修改权限
>
> customerId     isParent     （true就是主账号）==》 objId=350&editPermisson=1
>
> ```
> 规则标签用绿色表示就行，导入标签用灰色
> 绿
...
> https://jira.enncloud.cn/browse/ZNJY-6447
>
> 18900000090     
>
> trade：features/1116-membershipLevel
>
> ```
> /sass/labelSaas/customGroup?layout=pure&editPermisson=0
> 
> https://enn-label-manage-frontend.fat.ennew.com/#/labelSaas/customGroup/2?authCode=MTM0MDkxMjY3MDQ1MDEyNjkwMSNQQy1XRUIjNzM1NDk3MTI2ZDUzNzA2MmMxMzQ3ZjE2MjNhYWVhMGQ&authTenantId=1454333493678092289&from=trade&userId=1340912670450126901&companyId=B89C4DF61B27E067652E98930C7FDCCD&companyName=舟山接收站33&layout=pure&editPermisson=0&ojb=B89C4DF61B27E067652E98930C7FDCCD
> 
> https://enn-label-manage-frontend.fat.ennew.com/#/labelSaas/customGroup/2?authCode=MTM0MDkxMjY3MDQ1MDEyNjkwMSNQQy1XRUIjNzM1NDk3MTI2ZDUzNzA2MmMxMzQ3ZjE2MjNhYWVhMGQ&authTenantId=1454333493678092289&from=trade&userId=1340912670450126901&companyId=B89C4DF61B27E067652E98930C7FDCCD&companyName=舟山接收站33&layout=pure&editPermisson=0&objId=B89C4DF61B27E067652E98930C7FDCCD
> 
> https://enn-label-manage-frontend.fat.ennew.com/#/labelSaas/customGroup/2?authCode=MTM0MDkxMjY3MDQ1MDEyNjkwMSNQQy1XRUIjNTQ5ZWEyMWZmZDRiZWFhZGRhMzUzZDU1NTg2ZmIzZTY&authTenantId=1454333493678092289&from=trade&userId=1340912670450126901&companyId=B89C4DF61B27E067652E98930C7FDCCD&companyName=舟山接收站33&layout=pure&objId=B89C4DF61B27E067652E98930C7FDCCD&editPermisson=1
> 
> https://enn-label-manage-frontend.fat.ennew.com/#/labelSaas/customGroup/2?authCode=MTM0MDkxMjY3MDQ1MDEyNjkwMSNQQy1XRUIjMTY5ZDk5OWExZTk1NzRhZDhkNzRlOWZhMDk0NjhkYjM&authTenantId=1454333493678092289&from=trade&userId=1340912670450126901&companyId=B89C4DF61B27E067652E98930C7FDCCD&companyName=舟山接收站33&layout=pure&objId=B89C4DF61B27E067652E98930C7FDCCD&editPermisson=1
> 
> https://enn-label-manage-frontend.fat.ennew.com/#/labelSaas/customGroup/2?layout=pure&objId=B89C4DF61B27E067652E98930C7FDCCD&editPermisson=1&authTenantId=2898041747946594304&authCode=MTM4NjE0MTkyNjU0NTgwNTMxMyNQQy1XRUIjZjgxZGU3N2VjMWVkYzM0MzAzMDc4ZTNkMjE1N2MwY2M&userId=1386141926545805313
> 
> https://enn-label-manage-frontend.fat.ennew.com/#/labelSaas/customGroup/2?authCode=MTM0MDkxMjY3MDQ1MDEyNjkwMSNQQy1XRUIjMjhiNDIxMjExMmQxZWYxM2JhMGFhMGIwOGE5Zjg5MDI&authTenantId=1454333493678092289&from=trade&userId=1340912670450126901&companyId=B89C4DF61B27E067652E98930C7FDCCD&companyName=舟山接收站33&layout=pure&objId=B89C4DF61B27E067652E98930C7FDCCD&editPermisson=1
> 
> ```
>
> 18900000090     
...
![image-20231115100638719](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115100638719.png)

2 (完了)

![image-20231115100850989](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115100850989.png)

3 (完了--算了)

![image-20231115100926472](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115100926472.png)

4  改（完了）

![image-20231115101650862](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115101650862.png)

5   改（完了）

![image-20231115101810886](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115101810886.png)

6 (完了)

![image-20231115101905188](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115101905188.png)

7 （素拿了）

![image-20231115101941704](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115101941704.png)

8 （完了）

![image-20231115103122212](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115103122212.png)

9 不是（算了

![image-20231115103241469](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115103241469.png)

10  改 (完了)

![image-20231115103708384](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115103708384.png)

11（完了）

![image-20231115104001460](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115104001460.png)

12   改（完了）

![image-20231115104538365](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115104538365.png)

13（算了

![image-20231115105027032](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115105027032.png)

14 （完了）

![image-20231115105646561](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115105646561.png)

15

![image-20231115110139894](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115110139894.png)

16   改（完了）

![image-20231115110446451](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115110446451.png)

17 不是

![image-20231115111051704](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115111051704.png)

18  改(完了)

![image-20231115111232432](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115111232432.png)

19算了没事

![image-20231115111325471](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115111325471.png)

20 没事

![image-20231115134020489](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115134020489.png)

21      （完了

![image-20231115134257783](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115134257783.png)

22算了

![image-20231115135327162](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115135327162.png)

23 算了

![image-20231115135838483](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115135838483.png)

24 算了

![image-20231115140013487](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115140013487.png)

25

![image-20231115140410803](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20231115140410803.png)

# V3

> features/1219-pngorderExpansion
