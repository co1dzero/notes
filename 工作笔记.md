...
```js
小数位格式化的方法我已经写好提交了，
this.$decimalFormat(number, fixed, minLength)，
number 为需要格式化的数字（字符串或数字类型），fixed 为后端返回需要的小数长度，minLength 为最小小数长度（默认为2）
```
...
```js
{
    ...
    component: () => import(/* webpackChunkName: 'zhoushan' */ '@/views/zhoushan/info'),
  },
      {
    path: '/zhoushan-order',
    name: 'zhoushan_order',
    ...
    component: () => import(/* webpackChunkName: 'zhoushan' */ '@/views/zhoushan/order'),
  },
```

管道气预售，卖家挂牌输入最小交易量为0，页面提示修改

管道气预售，挂牌设置的最大交易量，买家下单手动输入购买量不能下单

```js
// 竞价
/manufacturer/newBidd
newPresaleListing.vue
// 预售
/new/preSaleListing
newBidd.vue - newActivity.vue

userDefinedCarryGasNumber
carryGasState
```
...
biddDetail.vue
// 竞拍详情--
export const newGetBidDetail = data => {
  return axios({
    url: `/new/open/api/factoryCenter/bid/getBidDetail/${data}`,
    method: 'get',
    data,
  });
};
...
```js
{
    path: '/manufacturer/contractList', // 厂家中心-年度框架合同
    name: 'manufacturerContractList',
    meta: {
      title: '年度框架合同',
      headerBarShow: true,
      leftSideShow: true,
    ...
    component: () =>
      import(/* webpackChunkName: 'buyer' */ '@/views/annualFrameContract/contractList'),
  },
 -------------------------------------------------------------------------------     
      //卖家中心-合同详情
      {
    path: '/manufacturer/contractDetail', // 厂家中心-年度框架合同详情
    ...
    meta: {
      ...
      menuShow: true,
      pPermisId: '100004',
      parents: ['100094', '100230'],
    },
    component: () =>
      import(/* webpackChunkName: 'buyer' */ '@/views/annualFrameContract/contractDetail'),
  },
/manufacturer/contractDetail?contractId=54ECA944C71903F63F0C172D8D2A621C
-----------------------------------------------------------------------------------------
{
    path: '/buyerCenter/buyerCreateContract',
    name: 'buyerCreateContract',
    meta: {
      title: '创建框架合同',
      headerBarShow: true,
      leftSideShow: true,
      menuShow: true,
      pPermisId: '100003',
      parents: ['100231', '100303'],
    },
    component: () =>
      import(/* webpackChunkName: 'buyer' */ '@/views/annualFrameContract/buyerCreateContract'),
  },

```

```
inputMode 假的录入方式参数需修改
files 上传的附件
defaultFileList 附件列表
remarksInformation 备注信息
```

![image-20230327165702366](C:\Users\shizeyu\AppData\Roaming\Typora\typora-user-images\image-20230327165702366.png)

017E53428B794DC183BEA1F494B3D5DF
...
```js
{
    path: '/buyerCenter/createPurchase',
    name: 'createPurchase',
    meta: {
      title: '发布招采',
      headerBarShow: true,
    ...
    component: () =>
      import(/* webpackChunkName: 'buyer' */ '@/views/buyer/myPurchases/creatPurchases'),
  },
```
...
![img](https://static.dingtalk.com/media/lQLPJxNhJDFNqarNA9TNB1awkv6SN2qDerwELreXHMACAQ_1878_980.png)

关于招采的地方，只要改一下时间格式 yyyy-MM-dd HH:mm，关于订单的地方



![img](https://static.dingtalk.com/media/lQLPJxyd0924KarNB_LNDCewtkD9GCZtIwUELreXG4CyAQ_3111_2034.png)

![img](https://static.dingtalk.com/media/lQLPJwrZcdcoeqrNBZnNCFSwEfuMBOp2NacELreXIACmAQ_2132_1433.png)

![img](https://static.dingtalk.com/media/lQLPJxp7OKwreqrNB4PNDSmwYSPt8Mssg0EELreXHMACAA_3369_1923.png)

![img](https://static.dingtalk.com/media/lQLPJxpyIFfC-arNBjzNCtqw3wpK_cF0GE8ELreXG4CyAA_2778_1596.png)

![img](https://static.dingtalk.com/media/lQLPJx4JoB-hKarNBynNCw2w2gmX9x3z97QELreXIACmAA_2829_1833.png)

![img](https://static.dingtalk.com/media/lQLPJwjJ8BEwOqrNBjvNCf-wTmsAdY8RshkELreXd0ACAQ_2559_1595.png)

![img](https://static.dingtalk.com/media/lQLPJxv5NYr5uqrNApXNBumwN3rM1z-MvxIELreXhwCyAA_1769_661.png)

![img](https://static.dingtalk.com/media/lQLPJwl_1jIkuqrNBcjNDd-wg983N6ZBr-IELreXkECmAA_3551_1480.png)

只有这些页面需要用 yyyy-MM-dd HH:mm的时间区间，别的都要还原成 许勇辉未开发之前的版本代码，之前应该不是取的 arrivalStartDate和arrivalEndDate字段，应该是取的别的字段，收货单和出货单里面应该有 判断的，去掉许勇辉加的判断就好了



/annualFrameContract

/sign-management-list
